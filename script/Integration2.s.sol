...
import {DeployHelpers, console} from "./DeployHelpers.s.sol";
...
contract Integration2 is DeployHelpers {
    ...
    function run() public {
        ...
        uint256 term2 = lendingManager.createLendingTerm(2, IHooks(address(0x1))); // 2% commission
        uint256 term3 = lendingManager.createLendingTerm(3, IHooks(address(0x2))); // 3% commission
        ...
        for (uint256 i = 0; i < borrowers.length; i++) {
            uint256 borrowerPk = vm.envUint(
                string(abi.encodePacked("BORROWER_PRIVATE_KEY_", i + 1))
            );
        ...
        for (uint256 i = 0; i < lenders.length; i++) {
            uint256 lenderPk = vm.envUint(
                string(abi.encodePacked("LENDER_PRIVATE_KEY_", i + 1))
            );
